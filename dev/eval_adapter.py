"""Load the trained LoRA adapter and run the same debug clip as baseline.

Writes eval_output.json for side-by-side comparison with baseline_output.json.
"""
...
OUT = Path(__file__).parent / "eval_output.json"
TRIALS = 3
...
def main():
    ...
    # Use the exact same prompt as baseline for fair A/B.
    messages = [{
        "role": "user",
        "content": [
